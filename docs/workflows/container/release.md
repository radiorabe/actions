...
6. Replace this with the actual name of the image, usually the name of your repo with maybe a
   `container-image-` prefix removed.
7. Replace the name with the stem of the image.
8. Put a human-friendly string into display-name.
9. Tags are usually `minimal rhel9 rabe` plus additional tags for the image at hand.

As a last step, it is recommended to add `trivy.*` to both your `.gitignore` and
`.dockerignore` files so Trivy can't interfere with multi-stage builds.
