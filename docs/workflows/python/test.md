...
Runs [pytest](https://pytest.org/) via [Poetry](https://python-poetry.org/) against the
specified Python version. The latest `3.x` release is always tested additionally as an
allowed failure.
...
3. The `version` input specifies the Python version to test against (defaults to `3.12`).
   The latest `3.x` is always tested additionally as an allowed failure.
